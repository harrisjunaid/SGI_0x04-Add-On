...
# 1) Setup directories and copying config-0x04.yaml on HAOS
# 2) Get configuration from the add-on and apply to config-0x04.yaml
...
if [ ! -f /share/SunGather/config-0x04.yaml ]; then
    cp config-hassio.yaml /share/SunGather/config-0x04.yaml
fi
...
" /share/SunGather/config-0x04.yaml
...
if [ $CUSTOM_MQTT_SERVER = true ]; then #custom MQTT details are in /share/SunGather/config-0x04.yaml
   echo "Skipping auto MQTT set up, please ensure MQTT settings are configured in /share/SunGather/config-0x04.yaml"
else
  ...
  if ! bashio::services.available "mqtt"; then
    ...
    " /share/SunGather/config-0x04.yam
...
exec python3 /sungather.py -c /share/SunGather/config-0x04.yaml -l /share/SunGather/ # -c is for config file, -l is for log file