...
class export_mqtt(object):
    ...
    def configure(self, config, inverter):
        ...
        self.mqtt_config = {
            ...
            'client_id': config.get('client_id', f'0x04{self.serial_number}'),
            'topic': config.get('topic', f"SunGrow-Inverter-0x04/{self.serial_number}"),
    ...
    def publish(self, inverter):
        ...
        if self.mqtt_config['homeassistant'] and not self.ha_discovery_published: # discovery is Fasle on first run
            ...
            ha_device = { 
                "name": f"0x04 {self.model} {self.serial_number}", 
                "identifiers": [f"0x04{self.serial_number}"], 
                ...
                #     ["port", inverter.getPort() or "0x04"]  # Assuming getPort() returns the correct port, otherwise handle it
                # ]
            }
            ...
            for ha_sensor in self.ha_sensors: # list provided at the top
                ...
                config_msg['unique_id'] = f"sungather_0x04_{self.cleanName(config_msg['name'])}_{self.serial_number}"
                ...
                ha_topic = f"homeassistant/{ha_sensor.get('sensor_type')}/0x04{self.serial_number}_{self.cleanName(ha_sensor.get('name'))}/config"  #Discovery Topic: <discovery_prefix>/<component>/<object_id>/config
